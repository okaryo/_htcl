...
The current implementation only detects redirect responses:
...
Following the redirect is a later step. That will need to decide how to resolve
relative `Location` values, when to preserve the original method and body, and
how many redirects are allowed before returning an error.
